...
class CrossAttention(nn.Module):
    def __init__(self, dim, context_dim, heads=8, dim_head=16):
        super().__init__()
    ...
    def forward(self, x, context):
        ...
        q = self.to_q(x).view(b, h, -1, l)
        k = self.to_k(context).view(b, n_ctx, h, -1).permute(0, 2, 3, 1)
        v = self.to_v(context).view(b, n_ctx, h, -1).permute(0, 2, 3, 1)
        ...
        out = out.reshape(b, -1, l)      # [B, inner_dim, L]
        return self.to_out(out)          # [B, dim, L]
...
class DINOv2Encoder(nn.Module):
    def __init__(self, embed_dim=128, finetune_last_n_layers=0, upsample_size=(224, 224)):
        super().__init__()
        # Loading DINOv2 config & model
        cfg = Dinov2Config.from_pretrained('facebook/dinov2-base')
        cfg.output_hidden_states = False
        cfg.return_dict = True
        self.encoder = Dinov2Model.from_pretrained('facebook/dinov2-base', config=cfg)
        
        # Projection head
        self.proj = nn.Linear(cfg.hidden_size, embed_dim)
        ...
        # Unfreeze projection and last N transformer blocks
    ...
    def forward(self, x: torch.Tensor):
        ...
        # Upsample to the size required by DINOv2
        x = self.upsample(x)

        outputs = self.encoder(pixel_values=x)
        tokens = outputs.last_hidden_state  # (B, num_patches+1, hidden_size)
        
        z = self.proj(tokens)              # (B, num_patches+1, embed_dim)
...
class Unet1DConditional(nn.Module):
    def __init__(
        ...
        for ind, (di, dout) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Residual(
                    PreNorm(di, CrossAttention(di, context_dim, heads=attn_heads, dim_head=attn_dim_head))
                ),
        ...
        self.mid_cross_attn = Residual(PreNorm(mid, CrossAttention(mid, context_dim, heads=attn_heads, dim_head=attn_dim_head)))
        ...
        for ind, (di, dout) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Residual(
                    PreNorm(dout, CrossAttention(dout, context_dim, heads=attn_heads, dim_head=attn_dim_head))
                ),
    ...
    def forward(self, x, time, z):
        ...
        h = []
        # down
        for b1, b2, attn, cross_attn, down in self.downs:
            x0 = b1(x0, t_emb); h.append(x0)
            x0 = b2(x0, t_emb)
            x0 = attn(x0)
            x0 = cross_attn(x0, z)  # 使用CrossAttention替代FiLM
            h.append(x0)
            x0 = down(x0)
        # mid
        x0 = self.mid_block1(x0, t_emb)
        x0 = self.mid_attn(x0)
        x0 = self.mid_cross_attn(x0, z)  # 添加中间层CrossAttention
        x0 = self.mid_block2(x0, t_emb)
        # up
        for b1, b2, attn, cross_attn, up in self.ups:
...
class ConditionalDiffusionModel(nn.Module):
    def __init__(self, embed_dim=128, unet_dim=64, seq_length=128, finetune_last_n_layers=2, diffusion_kwargs=None):
        super().__init__()
        diffusion_kwargs = diffusion_kwargs or {}
        self.encoder = DINOv2Encoder(embed_dim=embed_dim, finetune_last_n_layers=finetune_last_n_layers)
