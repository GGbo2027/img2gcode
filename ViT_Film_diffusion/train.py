...
from .diffusion import ConditionalDiffusionModel, ViTConfig, ViT
...
def visualize_predictions(model, data_loader, device, epoch, save_dir, debug: bool = False):
    ...
    num_samples = min(6, len(data_loader.dataset))
...
def train_conditional_diffusion(
    ...
    if resume_path and os.path.exists(resume_path):
        ...
        print(f" Resume training from checkpoints {resume_path}, start epoch: {start_epoch}")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for step, batch in pbar:
            ...
            tensors = batch["tensor"].to(device)  # [B,C,L]
            masks = batch.get("mask", None)  # [B,L]
...
from tqdm import tqdm
import argparse
from .diffusion import ConditionalDiffusionModel, ViTConfig


def main():
    parser = argparse.ArgumentParser(description="conditional ViT diffusion model training")
    parser.add_argument("--data_path", type=str, default="./filtered_dataset", help="dataset path")
    parser.add_argument("--save_dir", type=str, default="./checkpoints2", help="folder to save checkpoints")
    parser.add_argument("--batch_size", type=int, default=48, help="batch size")
    parser.add_argument("--image_size", type=int, default=64, help="img size")
    parser.add_argument("--max_tensor_len", type=int, default=128, help="max_tensor_len")
    parser.add_argument("--feature_dim", type=int, default=3, help="feature dim")
    parser.add_argument("--num_workers", type=int, default=12, help="workers")
    parser.add_argument("--epochs", type=int, default=500, help="training epochs")
    parser.add_argument("--lr", type=float, default=1e-5, help="learning rate")
    parser.add_argument("--device", type=str, default="cuda" if torch.cuda.is_available() else "cpu", help="device")
    parser.add_argument("--seed", type=int, default=42, help="Random seed")
    parser.add_argument("--resume", type=str, default=None, help="Resume from chekpoint")
    parser.add_argument("--eval_ratio", type=float, default=0.1, help="Val set ratio")
    parser.add_argument("--log_every", type=int, default=10, help="How often to log the loss")
    parser.add_argument("--save_every", type=int, default=5, help="How often to save the checkpoint")
    #parser.add_argument("--mask_weight", type=float, default=0.5, help="Weight of mask loss")
    
    # ViT
    parser.add_argument("--vit_width", type=int, default=512, help="ViT hidden size")
    parser.add_argument("--vit_layers", type=int, default=8, help="ViT layers")
    parser.add_argument("--vit_heads", type=int, default=8, help="ViT attention heads")
    parser.add_argument("--vit_patch_size", type=int, default=4, help="ViT patch size")
    
    # UNet
    parser.add_argument("--unet_dim", type=int, default=32, help="UNet base dim")
    parser.add_argument("--unet_dim_mults", type=str, default="1,2,2,4,6,8", help="UNet dim mutipliers")
    ...
    parser.add_argument("--objective", type=str, default="pred_x0", 
                        choices=["pred_noise", "pred_x0", "pred_v"], help="Traning objective")
    parser.add_argument("--beta_schedule", type=str, default="cosine", 
                        choices=["linear", "cosine"], help="beta")
    ...
    vit_model = ViT(vit_config, embed_dim=args.vit_width)
    diffusion_kwargs = {
    ...
    print(f"Start training, dataset path: {args.data_path}")
